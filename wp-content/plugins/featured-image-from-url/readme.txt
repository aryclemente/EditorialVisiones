...
#### FEATURED VIDEO
Supports videos from YouTube, Vimeo, Twitter, Imgur, 9GAG, Cloudinary, Tumblr, Publitio, JW Player, VideoPress, Sprout, Odysee, Rumble, Dailymotion, Cloudflare Stream, Bunny Stream, Amazon and BitChute. External and local video files are supported as well. 
...
= 4.3.0 =
* New option: Hide Featured Media > Post format filter; enhancement: Save in the Media Library (integration with Quick Edit); enhancement: Featured Video (supports BitChute videos now); enhancement: new integration function (fifu_dev_set_slider); fix: PHP Warnings.

= 4.2.9 =
* Enhancement: Auto set featured media using web page address (gets Amazon videos now); enhancement: WP-CLI (supports post creation now); enhancement: Featured Video (supports Amazon videos now); enhancement: new integration function (fifu_dev_set_video_list).

= 4.2.8 =
* New feature: Custom Popup; enhancement: Featured Video > Video Thumbnail (Bunny Stream videos have animated preview now); enhancement: FIFU Cloud (expiration time session increased); fix: FIFU Cloud > Automatic upload (trying to upload invalid images indefinitely); fix: security warning in Microsoft Edge (due JSON examples saved in txt files).
